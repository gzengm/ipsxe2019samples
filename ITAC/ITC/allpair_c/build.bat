...
rem ****************************************************************************
rem Content:
rem      Windows Build Script for ITAC: VT allpair C sample
rem ****************************************************************************
set clean=
set release=
set run=
...
if "x%clean%" == "xtrue" (
    ...
    del /S /Q allpair_c.stf* > nul 2>&1
    del /S /Q allpair_c.prot  > nul 2>&1
)
if "x%release%" == "xtrue" (
    @ECHO ON
...
if "x%run%" == "xtrue" (
    @ECHO ON
    mpiexec.exe -genv VT_DEADLOCK_TIMEOUT 5s -n 2 x64\Release\allpair_c.exe
    @ECHO OFF
)
